...
4.  `TELEGRAM_TOKEN`: Token của Telegram Bot (Tạo qua @BotFather).
5.  `TELEGRAM_TO`: Chat ID của bạn hoặc group Telegram (Có thể lấy qua @userinfobot).

### 🔄 Quy trình tự động:
1.  Bạn code xong và chạy: `git add .`, `git commit -m "..."`, `git push origin main`.
2.  GitHub Actions sẽ kích hoạt quy trình gồm 4 bước:
    - **Test & Build**: Kiểm tra lỗi cú pháp và chạy Unit Test.
    - **Security Scan**: Quét lỗ hổng bảo mật bằng Trivy.
    - **Deploy**: Kết nối vào EC2, `git pull` code mới và `docker compose up -d --build`.
    - **Notify**: Gửi thông báo thành công hoặc thất bại về Telegram.
